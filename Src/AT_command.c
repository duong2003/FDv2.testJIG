...
// Process AT command mode - handles command reading, processing, and response
bool AT_process_command_mode(char *command_buffer, char *response_buffer, int buffer_size, int timeout_ms)
{
    ...
    int command_length = usart2_read_line(command_buffer, buffer_size, timeout_ms);
    ...
    return false; // No command received (timeout or empty input)
}
